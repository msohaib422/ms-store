...
export default function ProductDetailPage() {
  ...
  const [reviewForm, setReviewForm] = useState({ customerName: '', rating: 5, review: '' });
  ...
  useEffect(() => {
    ...
    productsApi.getBySlug(slug)
      .then(res => {
        ...
        reviewsApi.getAll({ productId: p._id, approved: 'true' }).then(r => setReviews(r.data.data.reviews)).catch(() => {});
        if (p.category?._id) {
          productsApi.getAll({ category: p.category.slug, limit: 4 })
            .then(r => setRelated(r.data.data.products.filter(rp => rp._id !== p._id).slice(0, 4)))
            .catch(() => {});
        }
      })
  ...
  const finalPrice = hasVariants ? (selectedVariant.discountPrice || selectedVariant.price) : (product.discountPrice || product.price);
  ...
  const submitReview = async () => {
    if (!reviewForm.customerName.trim() || !reviewForm.review.trim()) { toast.error('Please fill all fields'); return; }
    setSubmitting(true);
    try {
      await reviewsApi.create({ ...reviewForm, product: product._id });
    ...
    <MainLayout>
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-neutral-500 mb-6">
          ...
          {product.category?.name && <><span>/</span><Link to={`/products?category=${product.category.slug}`} className="hover:text-primary-600">{product.category.name}</Link></>}
          <span>/</span>
        ...
        <div className="grid md:grid-cols-2 gap-10 mb-16">
          ...
          {/* Info */}
          <div>
            {product.category?.name && <Link to={`/products?category=${product.category.slug}`} className="text-sm text-primary-600 font-medium hover:underline">{product.category.name}</Link>}
            <h1 className="font-heading font-bold text-2xl md:text-3xl text-neutral-900 mt-2 mb-3">{product.name}</h1>
            <div className="flex items-center gap-3 mb-4">
              <div className="flex gap-1">{[1,2,3,4,5].map(s => <Star key={s} size={16} className={s <= Math.round(product.ratings) ? 'text-yellow-400 fill-yellow-400' : 'text-neutral-300 fill-neutral-300'} />)}</div>
            ...
            {product.shortDescription && <p className="text-neutral-600 mb-5">{product.shortDescription}</p>}
            <div className="grid grid-cols-2 gap-3 text-sm mb-6">
              {product.brand && <div className="flex items-center gap-2 text-neutral-600"><Tag size={14} className="text-primary-500" /> Brand: <strong>{product.brand}</strong></div>}
              {product.sku && <div className="flex items-center gap-2 text-neutral-600"><Package size={14} className="text-primary-500" /> SKU: <strong>{product.sku}</strong></div>}
        ...
        {product.description && (
          <div className="card p-8 mb-10">
            ...
            <div className="text-neutral-600 leading-relaxed whitespace-pre-wrap">{product.description}</div>
          </div>
        )}
        ...
        <div className="mb-10">
            ...
            <div className="grid md:grid-cols-2 gap-5 mb-8">
              {reviews.map(r => (
                <div key={r._id} className="card p-5">
                  ...
                  <p className="text-xs text-neutral-400 mt-2">{new Date(r.createdAt).toLocaleDateString()}</p>
                </div>
              ))}
            </div>
          )}
          <div className="card p-6">
            ...
            <div className="space-y-3">
              <input value={reviewForm.customerName} onChange={e => setReviewForm(f => ({...f, customerName: e.target.value}))} className="input" placeholder="Your name" />
